...
const TotalOpportunities: React.FC = () => {
  ...
  const latest =
    opportunitiesData.length > 0
      ? opportunitiesData[opportunitiesData.length - 1]
      : 0;

  const previous =
    opportunitiesData.length > 1
      ? opportunitiesData[opportunitiesData.length - 2]
      : 0;
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center text-violet-100">
        Loading opportunities analysis...
      </div>
    );
  }
