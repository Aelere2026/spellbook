...
const GainLoss: React.FC = () => {
  ...
  const { data: arbData, isLoading } = api.arbitrages.getWithMarkets.useQuery(
    undefined,
    {
  ...
  const chartData = useMemo(() => {
    const grouped = new Map<
      string,
      { label: string; gainLoss: number; gains: number; losses: number; date: Date }
    >();
    ...
    for (const a of arbitrages) {
      ...
      bucket.gainLoss = bucket.gains - bucket.losses;
    }
  ...
  const gainLossData = displayData.map((d) => d.gainLoss);

  const totalGains = arbitrages.filter((a) => Number(a.netProfit ?? 0) >= 0).length;
  const totalLosses = arbitrages.filter((a) => Number(a.netProfit ?? 0) < 0).length;
  const gainLossRatio = totalLosses === 0 ? totalGains : totalGains / totalLosses;

  const latest =
    gainLossData.length > 0 ? gainLossData[gainLossData.length - 1] : 0;

  const previous =
    gainLossData.length > 1 ? gainLossData[gainLossData.length - 2] : 0;
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-4">
              <MetricCard title="Total Gains" value={totalGains} isDark={isDark} />
              <MetricCard title="Total Losses" value={totalLosses} isDark={isDark} />
              <MetricCard
                title="Gain/Loss Ratio"
                value={gainLossRatio.toFixed(2)}
                isDark={isDark}
              />
              <MetricCard
        ...
        >
          ...
          >
            <LineGraph
              ...
              yAxisLabel="Gain/Loss Score"
              title={`Gain/Loss by ${scaleLabel}`}
              isDark={isDark}
            />
          </div>
        </section>
